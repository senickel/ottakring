...
reduce_data_and_temp_folder<-function(data,mean_var,over,subpop=NA,
  ...
  while(dir.exists(temp_folder)) temp_folder<-paste0(sample(c(letters,LETTERS,0:9),10),collapse="")
  dir.create(temp_folder)

  # shorten data to only necessary vars:
  vars<-svyset
  remove<-c("svyset","fpc(","_n","singleunit(","poststrata(","postweight(","strata(","scaled)")
  for (i in 1:length(remove)) vars<-vars %>% gsub(remove[i],"",.,fixed=TRUE)
  svyset_vars<-vars %>%
    gsub("[[:punct:]]","",.) %>%
    strsplit(" ") %>%
    unlist %>%
